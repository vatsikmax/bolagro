      ...
      <form action="" noValidate onSubmit={handleSubmit} onReset={handleReset}>
        <div className="flex items-center justify-between">
          ...
          {value && (
            ...
            >
              <XMarkMini />
              {dict.ControlledSearchBox.cancel}
            </button>
          )}
        </div>
      </form>
...
const SearchBox = ({ dict }: { dict: any }) => {
  const router = useRouter()
  return (
    <SearchBoxWrapper placeholder={dict.SearchBox.searchProducts}>
      {(props: any) => {
        return (
          <>
            <ControlledSearchBox {...props} dict={dict} />
          </>
        )
      }}
    </SearchBoxWrapper>
